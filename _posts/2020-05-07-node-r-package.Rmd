...
Mr [A Bichat](https://github.com/abichat) was looking for a way to bundle a NodeJS module inside n R package. 
...
On a production server, that also means that when publishing your app, you don't have to care about some global libraries, or about putting an app to prod with a module version that might break another application. 
...
+ Inside this folder, run `npm init -y` (the `-y` pre-fills all the fields), which creates `package.json`
...
+ Install external modules with `npm install module`, which add elements to `package.json`, and creates `package-lock.json` ; here, the whole module and its deps are downloaded and put inside a `node_modules/` folder
...
It can then be shared on `npm`, the Node package manager, or simply put on git, so that users `git clone` it, and install with `npm install` inside the folder. 
...
+ Then, from R, they will run an `npm install` inside the package installation folder, _i.e_ inside `system.file(package = "mypak")`
...
And here it is! 
With some extra package infrastructure, we've got everything we need :) 
...
Of course, one cool thing would be to test that `npm` and `Node` are installed on the user machine. 
We can do that by running a dummy node command, and check if the result of `system()` is either 0 or 127, 127 meaning that the command failed to run. 
