...
cond <- function(vec){
  ...
  adj_eq <- splt == lag(splt)
  incre <- splt >= lag(splt)
  any(na.omit(adj_eq)) & all(na.omit(incre))
}
vapply(test, cond, logical(1)) %>% sum()
```

    ## Error in vapply(test, cond, logical(1)) %>% sum(): could not find function "%>%"
...
cond2 <- function(vec){
  ...
  adj_eq <- na.omit(splt == lag(splt))
  if (any(adj_eq)){
    tb <- table(splt)
    adj_eq <- any(tb == 2)
  }
  incre <- na.omit(splt >= lag(splt))
  any(adj_eq) & all(incre)
}
vapply(test, cond2, logical(1)) %>% sum()
```

    ## Error in vapply(test, cond2, logical(1)) %>% sum(): could not find function "%>%"
